...
func _init() -> void:
	value = 9
	piece_name = "player_queen"
	next_piece_name = "player_king"
...
func calculate_possible_moves(board: Array, current_grid_location: Vector2, board_width: int, board_height: int) -> Dictionary:
	...
	# Define the possible directions that the Queen can move
	var move_directions := [ 
		Vector2(+0, -1),  # Up 
		Vector2(+1, +0),  # Right
		Vector2(+0, +1),  # Down
	...
	# Iterate through each possible direction the Queen can move
	for direction: Vector2 in move_directions:
		...
		# we need to reset it to the position of the Queen that wants to move
